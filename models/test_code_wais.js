...
module.exports = class DS_Test_Code_WAIS {
    ...
    insert() {        
        let strQuery = "INSERT INTO DS_Test_Code_WAIS (id_session, record_video) " +
                       "VALUES (?, ?) ";
    ...
    // Fields: [DS_Test_Code_WAIS] - id
    // Tables: [DS_Test_Code_WAIS]
    // Conditions: [DS_Test_Code_WAIS] - id_session    
    static select_by_idSession(idSession) {
        let strQuery = 'SELECT id ' +
                       'FROM DS_Test_Code_WAIS ' + 
                       'WHERE id_session = ? ';

        return db.execute(strQuery, [idSession]);        
    }

    // Fields: [DS_Test_Code_WAIS] - record_video
    // Tables: [DS_Test_Code_WAIS]
    // Conditions: [DS_Test_Code_WAIS] - id_session    
    ...
    static update_by_idSession(idSession, record_video) {
        let strQuery = "UPDATE DS_Test_Code_WAIS " +
                       "SET record_video = ? " + 
                       "WHERE id_session = ? ";
